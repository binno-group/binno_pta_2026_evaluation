...
its pre-deployment test programme, and the mobile app's buyer interface is
built against realistic mock data and is being wired to the backend API.

The product's UI design is in Figma:
[Binno design file](https://www.figma.com/design/IdmWngsrwdjgF6KLNfXPa2/Binno?node-id=0-1&p=f&t=d2RafpO9FknFhC51-0).
...
| 5 | 1 min | [binno_mobile/README.md](binno_mobile/README.md) | Mobile architecture, the design system, the encoded product rules |
| 6 | 2 min | [binno_mobile/lib/features/](binno_mobile/lib/features/) and [binno_mobile/lib/core/theme/](binno_mobile/lib/core/theme/) | Feature modules and a token-based design system instead of one-off styling |
...
binno_mobile/      Flutter application (buyer experience, mock-data stage)
  lib/core/        theme tokens, router, DI, map services, helpers
  lib/features/    feature modules: app, buyer, shared, gallery
  test/            unit tests
```

Size: the backend has 206 Go files (about 29,100 lines, 74 of them tests) and
27 SQL files; the app has 114 Dart files, about 14,300 lines, 5 of them test
files.
...
cd binno_mobile
flutter pub get
flutter run     # map screens need a Yandex MapKit key, see its README
flutter test
```
...
production takes every password from environment variables. The mobile app's
Yandex MapKit API key was replaced with a placeholder.
